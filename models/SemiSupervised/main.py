...
import os
import os.path
from models.SemiSupervised import log
from models.SemiSupervised import model
from models.SemiSupervised.util import image_save
...
def train(name, epochs, batch_size, labels, unlabels, noise_type, resume):
    ...
    log.info(log_path, {'_commandline': {
        'name': name,
        'epochs': epochs,
    ...
    def make_noise():
        if noise_type == 'normal':
            return numpy.random.randn(batch_size, model.Z_DIM)
        else:
            return numpy.random.normal(0, 1, size=[batch_size, model.Z_DIM])
    ...
    for _epoch in range(epochs):
        ...
        image_save(x_fake, '{base}/{epoch:03d}.{{i:03d}}.png'.format(base=result_dir, epoch=_epoch))
        log.write(log_path, {
            'epoch': _epoch + 1,
...
def test(snapshot):
    ...
    # model loading
    loaded_model = model.build()
    loaded_model.load_weights(snapshot)
